...
export function getRun(run_id) {
    return function (dispatch) {
        ...
        return axios.get(API_URL + RUNS_ENDPOINT, {
            params: {
            ...
            dispatch({type: FETCH_RUN_ERROR, payload: err});
        });
    };
}
...
export function createRun(pipeline_id, request_id) {
    return function (dispatch) {
        ...
        return axios(API_URL + RUNS_ENDPOINT, {
            ...
            dispatch({type: CREATE_RUN_ERROR, payload: err});
        });
    };
...
export function updatePorts(run_id, inputs, status) {
    return function (dispatch) {
        for (const key in inputs) {
            ...
            axios.put(API_URL + PORTS_ENDPOINT, {
                params: {
                    run_id: unescape(run_id),
                },
                headers: authHeader(),
                ...
                dispatch({type: UPDATE_PORT_ERROR, payload: err});
            });
        }
    };
}
...
export function updateRun(id, status) {
    return function (dispatch) {
        return axios.put(API_URL + RUNS_ENDPOINT + '/${id}', {
            params: {
                run_id: unescape(id),
            },
            headers: authHeader(),
            ...
            dispatch({type: UPDATE_RUN_ERROR, payload: err});
        });
    };
}
